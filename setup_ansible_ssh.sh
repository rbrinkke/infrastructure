...
# Doel: Installeer Ansible, configureer SSH-sleutels voor swarm_user, en verifieer de setup
...
# Controleer of het script wordt uitgevoerd als swarm_user
...
# Verifieer en update inventory.yml
echo "Vervang 'your_swarm_manager_ip' met '$SWARM_MANAGER_IP' in inventory.yml"
run_cmd "sed -i 's/your_swarm_manager_ip/$SWARM_MANAGER_IP/g' $INVENTORY_FILE"

# Stap 5: Controleer permissies van Ansible directory
echo "Controleer permissies van Ansible directory:"
ls -ld "$ANSIBLE_DIR"
ls -l "$ANSIBLE_DIR"

# Stap 6: Test Ansible verbinding met een simpele ping
